...
<template>
  <div class="font-display antialiased bg-background-light dark:bg-background-dark text-slate-900 dark:text-white transition-colors duration-300 min-h-screen flex items-center justify-center p-4">
    ...
    <main class="w-full max-w-[480px] animate-fade-in-up relative z-10">
      <div class="bg-white dark:bg-card-dark rounded-xl shadow-xl border border-slate-200 dark:border-border-dark overflow-hidden p-8 md:p-12 relative">
        ...
        <div class="flex flex-col items-center gap-6 mb-10">
          ...
          <div class="text-center space-y-2">
            ...
            <p class="text-slate-500 dark:text-slate-400 text-sm font-medium">Sign in to your dashboard</p>
          </div>
        </div>
        <!-- Google Sign In -->
        <button class="group w-full flex items-center justify-center gap-3 bg-white dark:bg-white text-slate-700 hover:text-slate-900 border border-slate-300 dark:border-transparent rounded-full h-12 px-6 transition-all duration-200 hover:shadow-md hover:bg-slate-50">
          ...
          <span class="text-sm font-bold tracking-wide">Sign in with Google</span>
        </button>
        <!-- Divider -->
        ...
        <form class="flex flex-col gap-4" @submit.prevent>
          <div class="space-y-4">
            <div class="relative group">
              ...
              <input class="block w-full rounded-full border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-[#111921] pl-11 pr-4 py-3 text-slate-900 dark:text-white placeholder:text-slate-400 focus:border-primary focus:ring-2 focus:ring-primary/20 dark:focus:ring-primary/30 sm:text-sm sm:leading-6 transition-all shadow-sm" id="email" name="email" placeholder="name@company.com" required type="email"/>
            </div>
        ...
        <!-- Footer / Terms -->
        <p class="mt-8 text-center text-xs text-slate-400 dark:text-slate-500 leading-relaxed max-w-xs mx-auto">
          ...
          and 
          <a class="underline decoration-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors" href="#">Privacy Policy</a>.
        </p>
      </div>
