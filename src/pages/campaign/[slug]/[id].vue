...
const testimonialId = parseInt(route.params.id as string)
...
const testimonial = computed(() => store.items.find(t => t.id === testimonialId))

const handleStatusUpdate = async (status: 'approved' | 'rejected') => {
    if (!testimonial.value) return
    await store.updateStatus(testimonialId, status)
    router.back()
}
</script>
...
<template>
  <div class="flex flex-col h-full bg-background-light dark:bg-background-dark overflow-y-auto">
    ...
    <header class="w-full flex items-center gap-4 px-8 py-6 bg-white dark:bg-[#111418] border-b border-slate-200 dark:border-[#283039] shrink-0 sticky top-0 z-10">
      ...
      <div v-if="testimonial" class="flex flex-col">
      ...
      <div v-if="testimonial" class="ml-auto flex gap-3">
        <button @click="handleStatusUpdate('rejected')" class="px-4 py-2 rounded-lg bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400 font-medium hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors">Reject</button>
        <button @click="handleStatusUpdate('approved')" class="px-4 py-2 rounded-lg bg-green-50 text-green-600 dark:bg-green-900/20 dark:text-green-400 font-medium hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors">Approve</button>
      </div>
    </header>

    <main class="flex-1 p-8 max-w-5xl mx-auto w-full flex flex-col gap-8">
       <!-- Testimonial Content Card -->
       <div v-if="testimonial" class="bg-white dark:bg-[#1c242c] rounded-2xl p-8 border border-slate-200 dark:border-[#283039] shadow-sm">
          <div class="flex flex-col md:flex-row gap-8">
             ...
             <div class="w-full md:w-1/3 shrink-0 flex flex-col gap-4">
                <div class="aspect-square rounded-2xl overflow-hidden bg-slate-100 dark:bg-[#283039] relative group">
                    <img v-if="testimonial.avatar" :src="testimonial.avatar" class="w-full h-full object-cover" alt="User avatar" />
                    <div v-else class="w-full h-full flex items-center justify-center text-slate-300 dark:text-slate-600">
                        <Icon icon="material-symbols:person" class="size-20" />
                    </div>
                </div>
                <div class="flex flex-col gap-2 p-4 rounded-xl bg-slate-50 dark:bg-[#283039]/50 border border-slate-100 dark:border-[#283039]">
                    <div class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
                        <Icon icon="material-symbols:campaign" class="size-4" />
                        <span class="truncate">{{ testimonial.campaignSlug }}</span>
                    </div>
                    <div class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
             ...
             <div class="flex-1 flex flex-col gap-6">
                ...
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="text-slate-500 dark:text-slate-400 font-medium">{{ testimonial.title }}</p>
                    </div>
       ...
       <div v-else class="text-center pt-20">
            <p class="text-slate-500">Loading testimonial...</p>
       </div>
    </main>
  </div>
</template>
